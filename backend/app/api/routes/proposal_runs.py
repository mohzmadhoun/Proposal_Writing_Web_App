...
def list_proposal_runs(
    ...
    stmt = select(ProposalRun).where(ProposalRun.organization_id == workspace.organization_id)
    if job_id:
...
def generate_proposal_run(
    ...
    job = db.scalar(
        ...
        .where(Job.id == payload.job_id, Job.organization_id == workspace.organization_id)
    )
    if not job:
